...
- The Phase 2 Flatpak workspace gate is `flatpak-builder --force-clean --disable-rofiles-fuse .flatpak-phase2 build-aux/flatpak/io.github.osv_ng.Phase2.yml`. Verify its installed stubs with `flatpak-builder --run .flatpak-phase2 build-aux/flatpak/io.github.osv_ng.Phase2.yml osv-app`, then the same command with `/app/libexec/osv-media-worker` and `/app/libexec/osv-archive-worker`. This offline bootstrap manifest is not release packaging and its temporary ID is not the product ID.
